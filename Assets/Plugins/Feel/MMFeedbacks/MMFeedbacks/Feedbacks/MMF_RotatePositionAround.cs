...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_RotatePositionAround : MMF_Feedback
	{
		...
		[Tooltip("the object around which to rotate AnimateRotationTarget, left empty is using Vector3.zero position")]
		...
		[Tooltip("if this is true, this become a spiral")]
		public bool IsASpirale = false;
		...
		protected virtual void ApplyRotation(Transform targetTransform, float remapZero, float remapOne, AnimationCurve curveX, AnimationCurve curveY, AnimationCurve curveZ, float percent)
		{
			...
			Vector3 AnimateRotationCenterPosition = AnimateRotationCenter ? AnimateRotationCenter.position : Vector3.zero;

			targetTransform.position = IsASpirale ? MMMaths.RotatePointInSpiralAroundPivot(targetTransform.position, AnimateRotationCenterPosition, _rotationAngles, percent) : MMMaths.RotatePointAroundPivot(targetTransform.position, AnimateRotationCenterPosition, _rotationAngles);
		}

...
{	
	...
	public static class MMMaths
	{
		...
		public static Vector3 RotatePointInSpiralAroundPivot(Vector3 point, Vector3 pivot, Vector3 angle, float percent)
		{
			// we get point direction from the point to the pivot
			Vector3 direction = point - pivot;
			// we rotate the direction
			direction = Quaternion.Euler(angle) * (direction * percent);
			// we determine the rotated point's position
			point = direction + pivot;
			return point;
		}
